...
export class GridComponent implements OnInit {
  ...
  constructor(private ts: TranslateService, private util: AppUtilBaseService, private http: HttpClient, private acRoute: ActivatedRoute, private baseService: BaseService, private appGlobalService: AppGlobalService, private maskService: MaskApplierService) {
    this.findPage();
  }
  findPage() {
  ...
  formatColumnDatas(col: any, data: any, type: any, row: any, meta: any, confRender: any = '') {
    ...
    switch (true) {
      ...
      case col.uiType == 'autosuggest' && !confRender:
        if (this.gridConfig.isSql) {
          formattedValue = row[col.field] ? this.getAutoSuggestValue(row, col) : '';
        } else {
  ...
  getAutoSuggestValue(data: any, col: any) {
    const displayFields = col.childFields;
    let displayFieldconCat: any = [];
    displayFields.map((o: any) => {
      let arr: any = [];
      if (data[o] && Array.isArray(data)) {
        data[o]?.forEach((k: any) => {
          arr.push(k);
        })
        arr.join('_');
        displayFieldconCat.push(arr);
      }
      else {
        displayFieldconCat.push(data[o]);
      }
    });
