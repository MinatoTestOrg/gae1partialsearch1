...
export class Marantable123DetailBaseComponent{
...
isSaving:boolean = false;
  ...
  "type" : "form"
	...
	pageViewTitle: string = 'MARANTABLE123_DETAIL';
...
step:workflowInfo.step
       ...
       const methodName: any = "configureFormOnWorkflow";
        let action: Exclude<keyof Marantable123DetailBaseComponent, ' '> = methodName;
...
editFormula(config:any,fieldname:any){
     ...
     if(fieldConfig.formulaMethod){
       let methodName: Exclude<keyof Marantable123DetailBaseComponent, ' '> = fieldConfig.formulaMethod
...
showMessage(config:any){
  ...
  const requestedObj = this.generateRequestObj(data);
      ...
      const saveSubscription = this.uploadAttachmentsandSaveData(requestedObj, splittedData).subscribe(res => {
        ...
        const saveSubscription = this.marantable123Service[method](requestedObj).subscribe((res:Marantable123Base) => {
          this.onAfterSave(res, data, method, isToastNotNeeded);
        }, err => { 
        this.isSaving = false;
         })
...
uploadAttachmentsandSaveData(data: any, splittedData: any): Observable<any> {
    ...
    if (!this.id) {
       const saveSubscription =  this.marantable123Service.create(splittedData.data).subscribe(
        createdData => {
    ...
    const subject$ = new Subject();    
      const updateSubscription = this.deleteFiles(splittedData).subscribe((responseData: any) => {
        this.saveFiles(responseData).subscribe((dataToUpdate: any) => {
          ...
          this.marantable123Service.update(dataToUpdate.data).subscribe(
            res => {
              subject$.next(res);
              subject$.complete();
            },
...
if (Array.isArray(ele)) {
  ...
  if (environment.prototype && this.id) {
    ...
    this.marantable123Service.getProtoTypingDataById(params).subscribe((res: any) => {
    ...
    const dataSubscription = this.marantable123Service.getById (params).subscribe((res: Marantable123Base[]) => {
      this.data = res || {};
      this.formatRawData();
    });
