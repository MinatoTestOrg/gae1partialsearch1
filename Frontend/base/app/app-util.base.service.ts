...
export class AppUtilBaseService {
  ...
  isValidForm(form: any, field: string, formErrors: any, finalArr: string[] = [], inValidFields: any = {}, fieldConfig?:any,parentForm?: string) {
   ...
   console.log(field);
    ...
    const fieldName = (fieldConfig[field]&&(fieldConfig[field]?.label !=""||fieldConfig[field]?.label !=undefined ))? (this.translateService.instant(fieldConfig[field].label)) : field;
  
      if (errors) {
        isValid = false;
        Object.keys(errors).forEach(errKey => {
          if (!formErrors[field]) {
            if(errKey === 'pattern' && formFieldConfig[field].regexError){
              formErrors[field] = formFieldConfig[field].regexError
            }
            else{
  ...
  findFormErrors(allErrors: any, finalArr: any,errorField:any) {
    for (const error in allErrors) {
      if (allErrors.hasOwnProperty(error)) {
        const element = allErrors[error];
        if(error === 'pattern' && errorField.regexError){
         finalArr.push(errorField.regexError);
        }
        else{
          finalArr.push(`
    ...
    if (config?.children?.length) {
      for (let index = 0; index < config.children.length; index++) {
        ...
        if (current?.children && current?.children.length) {
          ...
          if(config.children[index].uiType == 'email' && config.children[index]?.multipleValues){
